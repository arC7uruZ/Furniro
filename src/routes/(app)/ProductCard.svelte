<script lang="ts">
    ...
    let { imgSrc, imgAlt, title, description, discount, price, createdAt } =
        $props();
...
{/snippet}
<div>
    <a href="/" class="flex flex-col group w-71.25 h-111.5 relative">
        <img
            src={imgSrc}
            alt={imgAlt}
            width="285"
        ...
        />
        <div class="flex-1 flex flex-col justify-evenly p-4 bg-surface-subtle">
            <h4 class="font-primary font-semibold text-2xl">
                {title}
            </h4>
            <p class="font-primary font-medium text-base text-content-subtle">
                {description}
            </p>
        ...
        >
            ...
            <div class="flex justify-between text-content-on-action w-full">
